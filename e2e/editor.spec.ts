 ...
 *
 * Замер зондом (11.09.2026), задержка раздачи `/_next/static/**` на 3 с: в момент
 * «форма видна» `readyState=complete`, поле гидратировано, а в поле стоит название
 * ПРОШЛОГО блока; после `fill` — напечатанное; через 5 с — снова «Новый блок», и в
 * библиотеке лежит блок с этим именем. Без задержки то же окно закрывается мгновенно —
 * поэтому одиночный прогон был зелёным всегда, а в общем окно растягивается.
 *
