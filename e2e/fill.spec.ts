...
test.describe("язык документа на отказе по коду", () => {
   ...
   * на этот экран и попадают. Вторая: проверять надо то, что пришло ПО ПРОВОДУ, — его
   * и читают синтезатор речи с браузерным переводом. Пока рядом стоял клиентский
   * запасной рубеж (`HtmlLangSync`, снят в T272), в живой вкладке эта проверка была бы
   * зелёной поверх кривого документа.
   */
