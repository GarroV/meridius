...
/**
 * Меры, которые с описанием контура НЕ сверяются.
 *
 * Граница контура проходит по стране (см. `seed.ts`), а блок библиотеки страны не
 * имеет: он живёт сразу в нескольких чек-листах и заводится в общей библиотеке.
 * Заведённый на показе блок от рабочего неотличим, поэтому сид его намеренно не
 * снимает — а значит и требовать «блоков ровно столько, сколько в описании» нельзя:
 * на стенде с настоящим пакетом чек-листов смоук отказывался стартовать вовсе
 * («блоков библиотеки: в базе 7, в контуре 1», 17.09.2026, T206).
 *
 * Контроль не снят, он переехал с равенства на НЕИЗМЕННОСТЬ: `blocksChanged`.
 * Смоук отвечает за то, что убрал свои строки, а не за то, что в базе нет чужих.
 */
const OUTSIDE_CONTOUR = new Set<keyof Census>(["blocks"]);

/**
 * Изменилось ли за прогон число блоков библиотеки. Возвращает текст расхождения или
 * `undefined`. Прибавка означает, что смоук завёл блок и не убрал; пропажа — что снял
 * чужой, и это хуже: рабочий блок восстановить нечем.
 */
export function blocksChanged(
  ...
  return `блоков библиотеки было ${String(before)}, стало ${String(after)}`;
}
...
export function censusDifferences(actual: Census, expected: Census): string[] {
  return (Object.keys(CENSUS_TITLES) as (keyof Census)[])
    .filter((key) => !OUTSIDE_CONTOUR.has(key))
