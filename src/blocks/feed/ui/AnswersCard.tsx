      ...
      <table className={TABLE_CLASS} data-testid="submission-table">
        ...
        <tbody>
          {answer.rows.map((row, rowIndex) => (
            // Ключ строки — место плюс содержимое, а не одно содержимое: два
            // одинаковых замеса подряд («24 / 200» дважды) вполне обычное дело
            // для журнала, и на одном содержимом они дали бы React два одинаковых
            // ключа. Место здесь безопасно: список уже сохранён и не
            // переупорядочивается — в отличие от строк, которые сотрудник заводит.
            <tr
