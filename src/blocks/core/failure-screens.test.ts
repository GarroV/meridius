...
describe("граница ошибки продукта", () => {
  ...
  test("последний рубеж даёт то же, что корневая разметка: фон, шрифты, тему", () => {
    ...
    // Шрифты — из общего модуля, а не подключены заново и не забыты. Проверяется
    // ПРИМЕНЕНИЕ, а не имя: первая версия сторожа искала просто «FONT_VARIABLES» и
    // оставалась зелёной, когда класс с <html> убирали, — имя-то оставалось в строке
    // импорта. Проверка, которая не падает на испорченном коде, проверкой не является.
    expect(
      code,
      "последний рубеж не объявляет переменные шрифтов на <html>: текст наберётся системным",
    ).toContain("className={FONT_VARIABLES}");
