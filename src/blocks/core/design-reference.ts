...
/**
 * Заливка акцентом, записанная произвольным значением (`bg-[…]` с токеном акцента)
 * вместо утилиты `bg-accent`. Образец неправильной записи здесь НЕ приведён дословно
 * нарочно: сторож читает исходники блоков, и пример в комментарии он нашёл бы как
 * нарушение — ровно так эта проверка и покраснела на себе при заведении.
 *
 * Разницы на экране между ними нет — она есть в каскаде. Кольцо фокуса на акцентной
 * кнопке держится ОДНИМ правилом (`src/app/globals.css`, T280), и зацеплено оно за
 * класс `bg-accent`: другого признака «эта кнопка залита акцентом» у продукта нет,
 * а чинить фокус по кнопке значит чинить одну из шестнадцати. Кнопка, написавшая
 * заливку произвольным значением, из-под правила выпадает и возвращает дефект —
 * молча и на одном экране, пока сквозной сторож смотрит на другие.
 */
const RAW_ACCENT_FILL = /[\w:-]*\bbg-\[var\(--accent\)\]/g;

export function rawAccentFills(markup: string): readonly string[] {
  return markup.match(RAW_ACCENT_FILL) ?? [];
}
