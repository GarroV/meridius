 ...
 *
 * Два числа здесь не украшение, а две починки (T211 дефект #87, T222 дефект #99).
 *
 * `minmax(200px,1fr)` — пол поля названия. Прежний `1fr` означает `minmax(auto,1fr)`,
 * и у поля ввода собственная минимальная ширина почти нулевая: управление строки
 * забирало сколько просило, а название сжималось до 8 px на телефоне и до 112 px на
 * окне 1440, когда в строке сходились род «число» с границами, чип регулярности и
 * переключатель уровня. Пол в пикселях, а не в процентах: смысл — «влезает короткая
 * фраза», а он от ширины экрана не зависит.
 *
 * `minmax(0,auto)` — разрешение управлению быть уже своего содержимого. Без нижней
 * границы `auto` колонка требовала ширину всех кнопок в одну строку, и не влезшее
 * уезжало за край страницы целиком. С нулевым минимумом управление переносится
 * (`flex-wrap` ниже) — строка становится выше, а не шире экрана.
 *
 * Ниже складки (768 px, `--page-fold`) колонок две: управление переходит на свою
 * строку во всю ширину. Рядом с названием оно не помещается ни при каком поле: один
 * переключатель уровня занимает 257 px из 319 доступных на телефоне.
 */
const ROW_CLASS =
  "grid grid-cols-[28px_minmax(200px,1fr)_minmax(0,auto)] items-center gap-[var(--space-5)] px-[var(--space-6)] py-[var(--space-4)] max-md:grid-cols-[28px_minmax(0,1fr)]";
  ...
  return (
    ...
    >
      <div className={`${ROW_CLASS} ${showColumns ? "" : ROW_LINE_CLASS}`}>
        ...
        <div className="flex min-w-0 flex-wrap items-center justify-end gap-[var(--space-4)] max-md:col-span-2 max-md:justify-start">
          <select
