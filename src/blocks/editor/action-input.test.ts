...
describe("checklistInputFrom", () => {
  ...
  test("своё окно временем сильнее выбранного в списке (T185)", () => {
    // Демонстрационные данные содержат 05:00–17:00 — окно, которого в списке нет и
    // быть не может: список это быстрый выбор, а не перечень всех смен сети.
    expect(
      checklistInputFrom(
        form({
          window: "06:00|11:00",
    ...
    ).toStrictEqual({ start: "05:00", end: "17:00" });
  });

  test("недособранное своё окно не отменяет выбранное в списке", () => {
    expect(
      checklistInputFrom(form({ window: "20:00|00:00", windowFrom: "05:00" }))
        .window,
    ).toStrictEqual({ start: "20:00", end: "00:00" });
  });
