...
export interface FillItemView {
  ...
  /**
   * Единица измерения числового пункта («°C», «кг»), готовой строкой на языке
   * цепочки; `null` — единицы у пункта нет, и это обычное состояние, а не
   * недозаполненный пункт. Стоит РЯДОМ С ГРАНИЦАМИ у самого поля («-22…-10 °C»,
   * D110) и больше нигде на экране: назвать один факт дважды нельзя (T233).
   * Отдельным полем, а не приклеенной к `range` строкой: пункт без границ, но с
   * единицей иначе выдавал бы себя за пункт с диапазоном.
   */
...
/**
 * Признак того, что экран открыт в ПРИВЯЗАННОЙ ВКЛАДКЕ планшета станции, а не на телефоне
 * по наклейке (блок `device`, D132).
 *
 * Что он включает, сказано полем за полем ниже. Чего он не делает — так же важно: на
 * телефоне ничего из этого не появляется. Сброс недозаполненного на границе окна там
 * означал бы, что человек, заполняющий длинный чек-лист, теряет ответы без всякого повода.
 *
 * Готовые узлы, а не флаги: поведение вкладки (часы, удержание экрана, чип звука) живёт в
 * блоке `device`, а границы модулей не дают `fill` зависеть от него — и не должны. Экран
 * заполнения знает только, что у него есть чем это место занять.
 */
export interface TabletTab {
  /**
   * Опознаватель текущего окна времени. Уезжает в `key` формы: сменилось окно — React
   * снимает форму и ставит новую, и недозаполненный черновик уходит вместе с ней.
   * Обновления страницы для этого НЕ достаточно — черновик живёт в состоянии формы.
   */
  readonly windowKey: string;
  /** Поведение вкладки: часы до смены состояния, удержание экрана, чип звука. */
