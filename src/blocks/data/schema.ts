...
/**
 * Привязанные планшеты станций (блок `device`, D132, D133).
 *
 * Строка — это и есть право планшета показывать чек-лист: подписанная кука несёт только
 * опознаватель, и без живой строки она не значит ничего. Отвязка поэтому мгновенна и
 * выражена удалением, а не пометкой: журнала устройств пилоту не нужно, а мёртвая строка
 * в списке — тот же мусор, что и призрак привязки к удалённой станции.
 *
 * Владелец приходит через станцию → пиццерию → страну (D050, D051): своей колонки
 * владельца нет, а привязка к удалённой станции удаляется каскадом — хранить её нечего.
 */
export const devices = pgTable(
  "devices",
  {
    // Он же лежит в куке планшета. Случайный uuid: подобрать его нельзя, а подпись
    // куки не даёт подставить чужой.
    id: uuid("id").primaryKey().defaultRandom(),
    // Привязка знает СТАНЦИЮ, а не её код: код перевыпускается (D006), и вкладка берёт
    // свежий при каждой отрисовке — иначе перевыпуск ломал бы планшет.
    stationId: uuid("station_id")
      .notNull()
      .references(() => stations.id, { onDelete: "cascade" }),
    // Когда привязан: этим различаются два планшета одной станции в списке кабинета.
    pairedAt: serverTimestamp("paired_at"),
    // «Был на связи»: обновляется не чаще раза в несколько минут — запись при каждой
    // отрисовке превратила бы просмотр чек-листа в поток записей в базу.
    lastSeenAt: serverTimestamp("last_seen_at"),
  },
  (table) => [index("devices_station_idx").on(table.stationId)],
);

/**
 * Живые пины привязки: четырёхзначный код из кабинета, пять минут, один раз (D133).
 *
 * Одноразовость выражена колонкой `used_at` и проверяется в САМОМ запросе съедания
 * (`update … where used_at is null returning`): «прочитать, проверить, записать» здесь
 * обходится одновременными запросами — ровно так это уже случилось у входа в кабинет
 * (#93), и повторять это в коде доступа нельзя.
 *
 * Уникальность — частичным индексом среди несъеденных, а не словами «среди живых»:
 * `now()` для индекса не годится. Поэтому истёкшие несъеденные чистятся тем же запросом,
 * что выпускает новый пин, — иначе десять тысяч значений однажды кончатся и выпуск
 * начнёт отказывать на ровном месте.
 */
export const devicePairings = pgTable(
  "device_pairings",
  {
    ...
    // Съеден. `null` — пин ещё живой; именно по этому условию стоит уникальность.
    usedAt: timestamp("used_at", { withTimezone: true }),
  },
  (table) => [
    uniqueIndex("device_pairings_live_code_idx")
      .on(table.code)
      .where(sql`used_at is null`),
    index("device_pairings_station_idx").on(table.stationId),
    // Ровно четыре цифры и ничего кроме: длина кода — свойство продукта (D133), а не
    // договорённость вызывающего кода. Забытая проверка в коде не должна означать,
    // что база примет пин любого вида.
    check("device_pairings_code_shape", sql`code ~ '^[0-9]{4}$'`),
  ],
);
